...
export default class PrezlyApi {
  ...
  async getStoriesExtended(
    limit = DEFAULT_STORIES_COUNT,
    order: SortOrder = 'desc'
  ) {
    const stories = await this.getStories(limit, order)
    const extendedStoriesPromises = stories.map((story) =>
      this.getStory(story.id)
    )
  ...
  async getStoriesExtendedFromCategory(
    ...
    const extendedStoriesPromises =
      stories?.map((story) => this.getStory(story.id)) || []
