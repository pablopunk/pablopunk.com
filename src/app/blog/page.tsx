...
export default function BlogPage() {
  ...
  const filteredPosts = posts.filter(post => {
    ...
    for (const keyword of keywords) {
      ...
      if (post.title.toLowerCase().includes(keyword) || post.description.toLowerCase().includes(keyword)) {
        return true
      }
    }
      ...
      <div className="flex gap-2 mb-1">
        {
          keywords.map(keyword => (
            <button key={keyword} className={classNames('p-2 rounded-sm text-xs transition-colors hover:bg-indigo-600 hover:text-white ',
              {
